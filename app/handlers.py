...
class RoomSocketHandler(WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if action == "candidate": # if message received from one user
            candidate = data.get("candidate") # get the message
            if candidate:
                self.send_pair_message(action="candidate", candidate=candidate) #send it to other pair
        elif action == "desc":
            desc = data.get("desc")
            if desc:
                self.send_pair_message(action="desc", desc=desc) #send it to other pair
        elif action == "join":
            # Get the room_id
            try:
                ...
                # Tell both users that they are ready
                # self.send_message(action="ready", room_id=room_id)
                self.send_pair_message(action="ready", room_id=room_id)
        elif action == "new":
